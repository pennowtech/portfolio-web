  ...
  return (
    <div className="relative w-full h-[280px] md:h-[480px] overflow-hidden mb-8 left-0">
      ...
      <div className="w-full absolute text-white font-RobotoSlab flex flex-col items-end pr-2 drop-shadow-lg">
        <span className="text-xl leading-0 font-semibold">
          {month.slice(0, 3)}
        </span>
        ...
        <div className="absolute w-full bottom-0 text-white justify-center leading-4 md:max-w-screen-lg xl:max-w-[1167px] items-center mx-auto">
          <div className="my-4  text-sm md:text-base font-Comic  flex gap-x-4">
            {postMeta.tags.map((tag, idx) => (
              <div
                key={tag.id}
                className={`${bgColor[idx]}  rounded-full px-2 md:px-3`}
              >
                {tag.name}
              </div>
            ))}
          ...
          <div className="flex mb-4">
            ...
            <div className="relative">
              <h1 className="mb-2 font-Neuton font-semibold text-3xl md:text-6xl pr-4 text-slate-50">
                {postMeta.title}
              </h1>
