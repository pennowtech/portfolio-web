...
- app development
- series
thumbnailUrl: https://pennow.tech/wp-content/uploads/2021/05/master-slack-development.png
...
with a messaging app. Cutting long story short. Here, we will be discussing
...
fledged application.  __Disclaimer We'll not be discussing the slack app in
_general_ such as what it does. However, if required, I'll also touch on these
topics. But our whole discussion will be from a programming perspective. If
anyone wants to know how to use slack, then I'll suggest referring [tips and
tricks given by Maggie Tillman](https://www.pocket-
lint.com/apps/news/150925-what-is-slack-and-how-does-it-work-tips-tricks). She
explained it all in a very crispy yet concise way.
...
use any existing programming language with which you're comfortable. I'll be
using Python for our development purpose. It's not only easy to use but also
has a plethora of libraries, which anyone can imagine. So you don't have to
reinvent the wheel. Just concentrate on your work. Hence the only requirement
is: intermediate level of Python proficiency 😊
...
articles.  __Note We'll be using the _slack_bolt_ library officially released
...
that's deprecated by Slack.
  ...
  * [Step.1.](https://pennow.tech/slack-app-getting-started/) We will walk through setting up our development environment, configuring the slack app, and coding our app in Python.
  * [Step.2](https://pennow.tech/sending-messages-to-slack/). Send a plain text message to Slack or a complex one comprised of multiple sections. Also, discuss how to add our app to a channel.
  * [Step.3](https://pennow.tech/slack-app-with-python-handling-slack-events-part-3/). Listen and react to various events like messages, emojis, and reactions.
  * [Step.4](https://pennow.tech/slack-slash-commands/). Discussion on slash commands, a powerful way to send messages to Slack
  * [Step.5](https://pennow.tech/slack-publishing-ui-messages/). Publishing messages containing elements like buttons, select-menus, date-pickers, and more
  * [Step.6](https://pennow.tech/slack-handling-actions-from-slack-dialogues/). This article is in continuation of the previous article. Here we will be discussing how to handle actions triggered from visual elements such as button clicks or a date selection.
  * [Step.7](https://pennow.tech/slack-app-open-modal-views/). Modal dialogues where messages are displayed in visual blocks elements such as buttons, date picker, etc.
  * [Step.8](https://pennow.tech/slack-app-working-with-shortcuts/). Discussion about message shortcuts and global shortcuts, a convenient way to work with Slack.
  * [Step.9](https://pennow.tech/slack-app-working-with-message-shortcuts/). This article is in continuation of the previous article. Here we’ll discuss message shortcuts.
...
These are just a few add-ons. I just covered them as a reference section here:
  ...
  * [Scheduling messages to Slack](https://pennow.tech/scheduling-messages-to-slack/)
  * [Pin And Unpin Messages](https://pennow.tech/slack-app-pin-and-unpin-messages/)
  * Test your Slack App
...
> This section is a work-in-progress right now. I'll keep on updating this
> section as I add more and more articles under this series.
...
tickets, etc.
  ...
  * **Failure Notifications** : Message to be sent whenever an issue is faced with the following information: 
    ...
    * **Assign to:** To assign the issue to a user to debug and resolve it.  __Note Whenever any issue occurs, it will stop sending further notifications until the website is up again. However, one can run the job manually to check the current status with **/webmonitor run [job-id]** slash command. 
...
the quality of my articles. If you've any questions, please feel free to let
me know through your comments. Now let's start our journey ☕️

Image by [GraphicMama-team](https://pixabay.com/users/graphicmama-
team-2641041/) from [Pixabay](https://pixabay.com/)
