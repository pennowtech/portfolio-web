...
- **Failure Notifications** : Message to be sent whenever an issue is faced with the following information:
  ...
  - **Assign to:** To assign the issue to a user to debug and resolve it. \_\_Note Whenever any issue occurs, it will stop sending further notifications until the website is up again. However, one can run the job manually to check the current status with **/webmonitor run [job-id]** slash command.
