...
categories:
    - Kafka
date: 22 May, 21
description: "Kafka is full of surprises and that makes it a little confusing to understand
...
tags:
    ...
    - tutorials
    - Kafka
thumbnailUrl: /learning-apache-kafka.jpg
title: Learning Apache Kafka is Not Difficult Now

nextPostLink:  apache-kafka-architecture-and-essential-components
nextPostTitle: 'Apache Kafka Architecture And Essential Components'
nextPostImg: /apache-kafka-architecture-and-essential-components.jpg
prevPostLink: slack-app-pin-and-unpin-messages
prevPostTitle: "Slack App With Python: Pin And Unpin Messages [Extra]"
prevPostImg: /slack-app-pin-and-unpin-messages.svg
---

_**Kafka is full of surprises and that makes it a little confusing to understand in the beginning. However, it's not that too difficult to understand if you have the right material available with you. That's why I thought to note down my points and share them with you wonderful people.**_
...
you follow the principle of - _unlearn to learn_.
...
Once we're clear of Kafka fundamentals, we'll embark on our
...
a single article, we'll divide it into multiple articles.
...
- Kafka is based on the **[publish-subscribe patterns](/blog/learning-apache-kafka#the-beauty-of-the-publish-subscribe-pattern-)**. That means to send a message only once and received by multiple parties/consumers.
...
Thus, if you require any of these features for your projects, then Kafka
...
![pub-sub-pattern](/pub-sub-pattern.png)

We mentioned the publish-subscribe messaging system
a few times before. So I thought to take a halt to discuss this pattern before
moving ahead.

In **traditional queuing systems** , messages are sent
...
then no provision is provided by these queuing systems.

The publisher-consumer pattern solves both of these problems.

**Sender publishes** the messages on a
...
send a message once, and consumed by thousands.
...
In Kafka, this platform/channel is analogous to **topics**.

</section>

Apart from this, its publisher-subscriber pattern has some other benefits too:
...
Kafka can be extremely helpful.

- **Website core operations**
  Website activities like page views, clicks, searches, etc. can be published to Kafka channels in
  real-time. From there, it can be read by various consumers such as
  recommendation systems, data analytics, etc.

- **Communication between microservices**
  Kafka can be used as a communication medium between various
  microservices. I even feel it can replace microservice architecture in the
  future.

- **Aggregation of logs and events**
  Kafka can be used to collect logs
...
- **Stream Processing**
  You can even read data on the fly, _process_
...
- **Data collection from IoT sensors**
  Kafka is a popular choice to publish data from various sensors. You can even publish data to
...
Keep in mind that Kafka is not a replacement for the database. If
...
let's quickly summarize our discussion:

We started our discussion with a
...
In the next article, we'll start with the architecture of the Kafka
system and the various components. Bis bald 🍷

---

Title image credit:[Hintergrund Vektor erstellt von rawpixel.com - de.freepik.com](https://de.freepik.com/vektoren/hintergrund)
