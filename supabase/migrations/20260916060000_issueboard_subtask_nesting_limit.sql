-- Enforces the one-level subtask nesting limit (§6.5): a new issue's parent
-- must not itself already have a parent. Self-reference and cross-project
-- parents are already blocked by the table check constraint and the
-- existing project-membership check in this function, respectively.
...
begin
  ...
  if parent_issue_id_input is not null then
    select parent_issue_id into parent_of_parent from public.issueboard_issues
