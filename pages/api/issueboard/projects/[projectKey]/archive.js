import { setProjectArchived } from '@utils/issueboard/projectService';
import {
...
import { setProjectArchivedSchema, validationErrorResponse } from '@utils/issueboard/validation';
...
export default async function handler(req, res) {
  ...
  const parsed = setProjectArchivedSchema.safeParse(req.body);
  if (!parsed.success)
  ...
  try {
    ...
    const project = await setProjectArchived(projectKey, parsed.data.archived, actor.email);
    if (!project)
      return res
        .status(404)
        .json({ ok: false, requestId, error: { code: 'PROJECT_NOT_FOUND', message: 'Project not found.' } });
    return res.status(200).json({ ok: true, requestId, project });
  } catch (error) {
