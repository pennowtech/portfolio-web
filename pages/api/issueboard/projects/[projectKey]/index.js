import { updateProject } from '@utils/issueboard/projectService';
import {
...
import { updateProjectSchema, validationErrorResponse } from '@utils/issueboard/validation';

const projectKeyPattern = /^[A-Z][A-Z0-9]{1,9}$/;

export default async function handler(req, res) {
  const actor = await requireIssueboardAdminApi(req, res);
  if (!actor || !allowIssueboardMethods(req, res, ['PATCH'])) return;
  ...
  try {
    ...
    const project = await updateProject(projectKey, parsed.data, actor.email);
    if (!project)
      return res
        .status(404)
