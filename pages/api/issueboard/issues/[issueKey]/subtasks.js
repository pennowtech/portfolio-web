import { listSubtasks } from '@utils/issueboard/issueService';
...
export default async function handler(req, res) {
  ...
  try {
    const subtasks = await listSubtasks(match[1], Number(match[2]));
    if (subtasks === null)
      return res
        .status(404)
        .json({ ok: false, requestId, error: { code: 'ISSUE_NOT_FOUND', message: 'Issue not found.' } });
    return res.status(200).json({ ok: true, requestId, subtasks });
  } catch (error) {
